...
DUMMY_EXAMPLE_INPUT = dict(
    instruction="Solve:", output_1="2", input="1+1=", output_2="3"
)
...
class PaddingForBatchesProcessorInputNoinput(eval_processors.BaseProcessor):
    def __init__(
        self,
        batch_size,
        padding_example_input: dict,
        padding_example_noinput: dict,
        **kwargs,
    ):
    ...
    def preprocess(self, df_to_annotate: pd.DataFrame) -> pd.DataFrame:
        ...
        if arr_is_inputs.any():
            ...
            padded_df_without_inputs = self.padded_noinput.preprocess(
                df_to_annotate[~arr_is_inputs].copy()
            )
            padded_df_with_inputs = self.padded_input.preprocess(
                df_to_annotate[arr_is_inputs].copy()
            )

            df_out = pd.concat(
                [padded_df_without_inputs, padded_df_with_inputs],
                axis=0,
                ignore_index=True,
            )
        else:
