...
def ignore_tokens(
    input_ids: Tensor, attention_mask: Tensor, tokens_to_ignore: Sequence[int]
):
...
class DataPool(object):
    ...
    def sort_and_get(self, train_on_best_quantile=True):
        ...
        size = len(queries)
        chunk_sizes = [size // len(self.additional_special_tokens)] * len(
            self.additional_special_tokens
        )
        chunk_sizes[-1] = chunk_sizes[-1] + size % len(self.additional_special_tokens)
        assert (
            sum(chunk_sizes) == size
        ), "Internal error: Sum of chunk sizes doesn't match up with total size."

        if train_on_best_quantile:  # Don't inject any tokens here.
            queries, responses, rewards = tuple(
                l[: chunk_sizes[0]] for l in (queries, responses, rewards)
            )
        else:
            injected_tokens = []
            for chunk_index, chunk_size in enumerate(chunk_sizes):
                injected_tokens.extend(
                    [self.additional_special_tokens[chunk_index]] * chunk_size
                )
            queries = [
                f"{injected_token}{query}"
                for injected_token, query in utils.zip_(injected_tokens, queries)
            ]
...
class QuarkTrainer(rl_trainer.RLTrainer):
    ...
    def train(self):
        ...
        for step_idx in tqdm.tqdm(
            ...
            if (
                ...
                self.save_model(
                    utils.join(self.args.output_dir, f"checkpoint-{step_idx}")
                )
            if (
    ...
    def step(self, train_dataloader, step_idx, **kwargs):
        ...
        stats_list = []
        for _ in tqdm.tqdm(
            range(self.args.num_gradient_steps_per_step),
            disable=not self.accelerator.is_main_process,
            desc="gradstep",
        ):
            for substep_idx in range(
                1, self.accelerator.gradient_accumulation_steps + 1
            ):
                ...
                should_sync = (
                    substep_idx == self.accelerator.gradient_accumulation_steps
                )
                context = (
                    contextlib.nullcontext if should_sync else self.accelerator.no_sync
                )
    ...
    def compute_loss(
        ...
        with torch.inference_mode():
            ref_policy_outputs = self.ref_policy(
                queries_no_quark,
                query_attn_masks_no_quark,
                responses,
                temperature=self.args.temperature,
            )

        logits, logprobs = common.unpack_dict(
            policy_outputs, keys=("logits", "logprobs")
        )
        ...
        logits, ref_logits = tuple(
            t[..., :original_vocab_size] for t in (logits, ref_logits)
        )
        ...
        loss = (
            -logprobs * logprobs_coef
            + kl_per_token * kl_coef
            - entropies * entropy_coef
        )
        loss = loss.mean()
    ...
    def rollout(
        self, train_dataloader: utils.InfiniteLoader, step_idx: int
    ) -> utils.InfiniteLoader:
        ...
        for batch_idx in tqdm.tqdm(
            range(self.args.rollout_accumulation_steps),
            disable=not self.accelerator.is_main_process,
            desc="rollout",
        ):
            ...
            ):  # Must ignore the reward token on first generation.
            ...
            self.data_pool.add(
                queries=text_queries, responses=text_responses, rewards=rewards.tolist()
            )
        ...
        if self.accelerator.is_main_process:
            rollouts_to_disk = {
                "queries": text_queries_all,
                "responses": text_responses_all,
                "rewards": rewards_all,
            }
            ...
            self.accelerator.log(
                {"train/reward": utils.mean(rewards_all)}, step=step_idx
            )

        text_queries, text_responses, _ = self.data_pool.sort_and_get(
            self.args.train_on_best_quantile
        )
...
def make_tokenizer(args):
    ...
    logger.warning(
        f"Adding {args.num_reward_tokens} reward conditioning tokens for Quark."
    )
    policy_tokenizer.add_special_tokens(
        {
            "additional_special_tokens": [
                f"<reward_{i}>" for i in range(args.num_reward_tokens)
            ]
        }  # noqa
    )
...
def make_models(
    ...
    def make_generative_policy():
        ...
        utils.stable_resize_token_embeddings(
            base_model, len(tokenizer), jitter_new_embeddings=True
        )
        return base_model
    ...
    policy = common.prepare_model_for_custom_fn(
        model=policy, fn_name="respond", accelerator=accelerator
    )
