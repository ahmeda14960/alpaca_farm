...
class RLTrainer(object):
    ...
    def _make_fsdp_happy(self):
        ...
        if self.accelerator.distributed_type == DistributedType.FSDP:
            inputs = self.tokenizer(
                "fsdp are you happy now? :)" * 50, return_tensors="pt"
            )
            inputs = common.prepare_inputs(inputs, device=self.accelerator.device)
            self.policy(
                inputs["input_ids"], inputs["attention_mask"], inputs["input_ids"]
            )
    ...
    def evaluate(self, step_idx: int, unwrapped_policy=None):
        ...
        if self.accelerator.is_main_process:
            ...
            if self.args.output_dir is not None:
                utils.jdump(
                    results,
                    utils.join(self.args.output_dir, f"eval_results_{step_idx}.json"),
                )
    ...
    def get_train_dataloader(self):
        logger.warning(
            f"Train dataset size: {len(self.train_dataset)}", main_process_only=True
        )  # noqa
